...
Microflow is a minimalistic orchestration framework to run your data pipelines or other automated jobs, written in Python.
It runs primarily as a server and triggers jobs based on its internal schedules or external api calls. It features a web app which allows monitoring executions and running jobs manually.


It aims to be minimalistic in a sense that it does not use a lot of resources and compute time on top of the actual jobs you orchestrate, and has a very simple API which makes it easier to use. It works well on Unix and Windows platforms.


This page illustrates the full API of Microflow. Most features are optional and can be used together. As you can combine behaviors in a very flexible manner, we believe that this framework should cover most use cases.
...
```Bash
microflow my_flow:flow --host localhost --port 3000
```
...
from microflow import Flow, ConcurrencyGroup
